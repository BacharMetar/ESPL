...
void print_program_header(Elf32_Phdr *phdr, int index) {
    // printf("Program header number %d at address 0x%x:\n", index, phdr->p_vaddr); // Implementation of first print version
...
void load_phdr(Elf32_Phdr *phdr, int fd) {
    ...
    if (phdr->p_type == PT_LOAD) {
        ...
        void *mapped_address;
        off_t vaddr = phdr->p_vaddr&0xfffff000;
        off_t offset = phdr->p_offset&0xfffff000;
        size_t padding = phdr->p_vaddr & 0xfff;
        // Debug prints:
        // printf("Original Vaddr: %p\n", (void*)phdr->p_vaddr);
        // printf("Vaddr: %p\n", (void*)vaddr);
        // printf("Original Offset: %d\n", phdr->p_offset);
        // printf("Offset: %ld\n", offset);
        // printf("Padding: %d\n", padding);
        mapped_address = mmap((void*)vaddr, phdr->p_memsz + padding, protection_flags, MAP_PRIVATE | MAP_FIXED, fd, offset);
...
int main(int argc, char *argv[], char *envp[]) {
    if (argc < 2) {
        printf("Usage: ./loader <elf-filename> <args>\n");
        return 0;
    }
    ...
    lseek(fd, 0, SEEK_SET);
    void *start_address = mmap(NULL, file_size, PROT_READ, MAP_PRIVATE, fd, 0);
    ...
    foreach_phdr(start_address, load_phdr, fd);

    // Call the startup function with appropriate arguments
    Elf32_Ehdr *elf_head = (Elf32_Ehdr *) start_address;
    // Debug prints:
    // printf("Mapped start address: %p\n", (void*) start_address);
    ...
    // Debug print to show the linking with start.o was successful:
    // char *message = "Debug: Linked with start.o executable successfully\n";
